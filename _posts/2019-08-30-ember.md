...
export default ApplicationAdapter.extend({
    ...
    query(store, type, query) {
        let url = this.buildURL(type.modelName, null, null, 'query', query);
        delete query.id; //manipulate the query object in the query method 
        return this.ajax(url, 'GET', { data: query }); //pass it along
    },
}
```

The method `query()` is the first one called. Then it's modfiedied and passed along with return value of the ajax call. We do no expect it's being used elswhere. 

One may still argue that we are still modifying input parameter, yes we are. So to be extra safe we could copy the object to avoid any side effects, but I don't feel it's neccessary in this very case. 
